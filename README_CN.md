...
在首字延迟（TTFT）方面，测试数据揭示了一个反直觉的现象：尽管优化后输入 token 大幅减少，但 TTFT 却略微变长（优化后平均 914ms，原版平均 867ms）。这并非异常，而是架构设计的必然结果。大模型处理上下文的耗时并非只取决于 token 数量，更取决于**指令复杂度**（Cognitive Load）。原版提示词只是单纯的 JSON 历史数据堆砌，模型扫过这类同质化数据的速度极快；而优化后的 Prompt 则由极高密度的强约束规则（System Prompt）和复杂的 Few-shot 示例组成。大模型（尤其是 Claude 系列）的注意力机制需要花费更多时间去理解并严格遵循这些结构化约束。这正是确定性压缩的核心交换逻辑——我们用微不足道的前置理解成本，彻底阻断了上下文膨胀，并最终在端到端总延迟上赢回了 16.2% 的显著优势。
