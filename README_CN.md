...
在首字延迟（TTFT）方面，两者的表现基本持平（优化后平均 914ms，原版平均 867ms）。这验证了我们的核心观点：这套架构的目的并非在所有指标上寻求单点突破，而是在保证响应速度的前提下，提供一种极其稳定的规模化方案。当输入变得非平凡时，确定性压缩可以有效控制上下文的无序增长，显著降低端到端的总延迟。
